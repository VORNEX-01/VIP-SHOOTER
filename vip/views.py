...
اینجانب ([نام و نام خانوادگی]) ، با آگاهی کامل از شرایط و نحوه برگزاری پلن VIP کوچینگ 30روزه، در سلامت کامل جسمی و روحی و بدون هیچ‌گونه اجبار یا فشاری، با رضایت و اختیار شخصی خودم در این پلن شرکت می‌کنم. می‌دونم که در این دوره، آموزش‌ها، راهنمایی‌ها و مسیر لازم برای رشد و بهبود پیجم در اختیارم قرار می‌گیره و نتیجه گرفتن از این مسیر، به میزان تلاش، پیگیری و اجرای درست آموزش‌ها توسط خودم بستگی داره. بنابراین می‌دونم که مسئولیت اجرای آموزش‌ها و اقداماتی که برای رشد پیجم لازمه، بر عهده خودمه و اگر در اجرای این مسیر کوتاهی کنم یا آموزش‌ها رو به‌درستی انجام ندم، مسئولیت نتیجه نگرفتن یا عدم دستیابی به نتیجه مطلوب بر عهده خودم خواهد بود و این موضوع به معنای عدم تعهد یا کوتاهی از سمت برگزارکننده نیست.
""").strip()
...
def home(request, token=None):
    ...
    try:
        name_for_text = (last_name or "").strip() or "[نام و نام خانوادگی]"
        commit_text = COMMIT_TEXT_TEMPLATE.format(name=name_for_text)
    except Exception:
        commit_text = COMMIT_TEXT_TEMPLATE.replace("{name}", "[نام و نام خانوادگی]")
